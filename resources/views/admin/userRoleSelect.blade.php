    ...
    <div class="ibox-content">
            <div class="form-group">
                <div class="col-sm-8">
                    ...
                    <input type="hidden" name="id" class="id" value="{{$admin_id}}">
                        @if($y == "")
                            @foreach($res as $val)
                                <label><input type="checkbox" value="{{$val['rule_name']}}" name="rule_name"> <i></i>{{$val['rule_name']}}</label>
                            @endforeach
                        @else
                            @foreach($y[1] as $val)
                                <label><input type="checkbox" value="{{$val}}" checked="" name="rule_name"> <i></i>{{$val}}</label>          
                            @endforeach
                            @foreach($y[0] as $v)
                                <label><input type="checkbox" value="{{$v}}" name="rule_name"> <i></i>{{$v}}</label>
                            @endforeach
                        @endif
...
<script>
   $(".btn").click(function(){
        var param = [];
        $("input[name='rule_name']:checkbox:checked").each(function(i,o){
            param[i] = $(o).val();
        });
        var id = $(".id").val();
        $.ajax({
            type:"get",
            url:"{{ URL('admin/addUserRoleSelect') }}",
