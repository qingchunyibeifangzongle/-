//imglimouseover,imglimouseout,imglidel这3三个函数用于删除上传后的图片
...
$(function(){
    var $wrap = $('#uploader'),
        ...
        if ( !WebUploader.Uploader.support('flash') && WebUploader.browser.ie ) {
            ...
            if (flashVersion) {
                (function(container) {
                    ...
                    var swf = '__PUBLIC__' +'/plugins/webuploader/expressInstall.swf';
                    // insert flash object
            ...
            layer.msg( 'Web Uploader 不支持您的浏览器！', {icon: 5});
            return;
        }

        //判断定义SAVE_DIR，UPLOAD_WIDTH，UPLOAD_HEIGHT，MAX_NUM
        if(typeof(SAVEDIR) == 'undefined'){SAVEDIR='goods'} else{};
        if(typeof(UPLOADWIDTH) == 'undefined'){ UPLOADWIDTH=0} else{};
        if(typeof(UPLOADHEIGHT) == 'undefined'){UPLOADHEIGHT=0} else{ };
        if(typeof(MAXNUM) == 'undefined'){ MAXNUM=30} else{};
        // 实例化
        uploader = WebUploader.create({
            ...
            //chunked: true,
            //chunkSize: 512 * 1024,
            server:  Think.U('Home/upload/uploadpic') ,
            // runtimeOrder: 'flash',
            fileVal:'realeaseimg',
             accept: {
                 title: 'Images',
        ...
        function addFile( file ) {
            //判断如果图片数量超出了指定最大值，不添加进预览列表中
            if($('.filelist li').length >=MAXNUM){
                return ;
            }
        ...
        function updateStatus() {
            ...
            if ( state === 'ready' ) {
                ...
                if ( stats.uploadFailNum ) {
                    text = '已成功上传' + stats.successNum+ '张照片至图片相册，'+
        ...
        uploader.onUploadSuccess=function(file,response) {
            ...
            var obj = eval("(" + oRet + ")");
            $('.progress').hide();
            if($('input[name="image5"]').length==0){
                imageNum = 5;
            }
            if($('input[name="image4"]').length==0){
                imageNum = 4;
            }
            if($('input[name="image3"]').length==0){
                imageNum = 3;
            }
            if($('input[name="image2"]').length==0){
                imageNum = 2;
            }
            if($('input[name="image1"]').length==0){
                imageNum = 1;
            }
            $('#'+file.id).append('<input type="hidden" id=image'+ imageNum +' name=image'+ imageNum +' class="gallery-img" value="' +response.savepath + response.savename+'"/>');
        ...
        uploader.startUpload = function(){
            //loading = layer.load({time: 12*1000});
        }
        ...
        uploader.onError = function( code ) {
            layer.msg( '图片重复或超出了最大上传数量', {icon: 5});
        };
